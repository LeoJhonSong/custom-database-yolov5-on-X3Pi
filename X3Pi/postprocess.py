...
def postprocess(model_output,
    ...
    org_height, org_width = (672, 672)

    process_height, process_width = model_hw_shape

    pred_sbbox = model_output[0].reshape([1, 84, 84, 3,
                                          10]).transpose([0, 3, 1, 2, 4])
    pred_mbbox = model_output[1].reshape([1, 42, 42, 3,
                                          10]).transpose([0, 3, 1, 2, 4])
    pred_lbbox = model_output[2].reshape([1, 21, 21, 3,
                                          10]).transpose([0, 3, 1, 2, 4])
    ...
    for i, bbox in enumerate(bboxes):
        ...
        # print("{}: {:.4f}".format(classes_name, score))
        if classes_name not in cls_conf.keys() or cls_conf[classes_name] < score:
            cls_conf[classes_name] = score
    if len(cls_conf):
        print(cls_conf)
        return sorted(cls_conf.items(), key=lambda x: x[1], reverse=True)[0][0]
    else:
        return ''


def get_classes(class_file_name='traffic_classes.names'):
